...
with open('src/components/ConsultationBanner.tsx', 'r') as f:
    content = f.read()

new_banner = """import React, { useState } from 'react';
    ...
    <div className="w-full bg-[#032f6a] text-white py-3 px-4 flex justify-center">
      <div className="max-w-[1200px] w-full flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          <a
            href="tel:0348402466"
            className="flex items-center gap-2 border border-[#fdb913] rounded-full px-4 py-1.5 hover:bg-white/10 transition"
          >
            <Phone className="w-4 h-4 text-[#fdb913] fill-[#fdb913]" />
            <span className="font-bold text-lg text-[#fdb913]">034 84 02466</span>
          </a>
        </div>
...
with open('src/components/ConsultationBanner.tsx', 'w') as f:
    f.write(new_banner)
