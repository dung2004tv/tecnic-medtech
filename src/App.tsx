            ...
            {currentView === 'PRODUCTS' && (
              <div id="product-catalog-grid" className="space-y-4 w-full">
                ...
                <div className="flex flex-row items-start gap-2 sm:gap-4 md:gap-5 w-full">
                  ...
                  {/* CỘT BÊN TRÁI: BỘ LỌC THU GỌN VỪA VẶN TRÊN MOBILE ĐỂ NHƯỜNG KHÔNG GIAN CHO SẢN PHẨM */}
                  {showLeftFilter && (
                    <div className="w-[140px] xs:w-[155px] sm:w-56 md:w-64 lg:w-72 shrink-0 sticky top-20 z-20 self-start max-h-[calc(100vh-5.5rem)] overflow-y-auto overscroll-contain pr-0.5 sm:pr-1 custom-scrollbar">
                      <MetaSidebarFilter
                        activeCategory={activeCategory}
                        onSelectCategory={(catId) => handleSelectCategory(catId)}
                    ...
                    {isHomeDefault ? (
                      ...
                      <div className="space-y-6">
                        ...
                        <section className="bg-gradient-to-r from-[#143472] to-[#0071ba] rounded-3xl p-5 sm:p-6 text-white shadow-md relative overflow-hidden">
                          ...
                          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-2.5 sm:gap-3">
                            {bestSellerProducts.slice(0, 4).map((prod) => (
                              ...
                              >
                                <div className="relative flex items-center justify-center h-28 xs:h-32 rounded-xl overflow-hidden mb-1.5 bg-slate-50">
                                  <ProductImage product={prod} size="sm" showBadge={false} />
                                </div>
                                
                                <div className="space-y-1">
                                  <h5 className="font-bold text-[12px] sm:text-xs text-slate-900 line-clamp-2 min-h-[32px] leading-tight group-hover:text-[#0071ba]">
                                    {prod.name}
                                  </h5>
                                  <div className="pt-1 flex items-center justify-between gap-1">
                                    <span className="font-black text-xs xs:text-sm text-red-600 whitespace-nowrap">
                                      {prod.tecnicPrice.toLocaleString('vi-VN')}&nbsp;đ
                                    </span>
                                    <span className="text-[9px] xs:text-[10px] text-emerald-700 font-bold bg-emerald-50 px-1.5 py-0.5 rounded shrink-0">
                                      Bán chạy
                                    </span>
                                  </div>
                                </div>
