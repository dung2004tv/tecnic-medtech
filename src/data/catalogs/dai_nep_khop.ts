import { Product } from '../../types';

export const DAI_NEP_KHOP_PRODUCTS: Product[] = [
  {
    "id": 100034,
    "code": "TEC-100034",
    "name": "Đai Cổ Tay Wrist Bandage – Nẹp Cổ Tay Hỗ Trợ Cố Định Và Bảo Vệ Cổ Tay",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 230000,
    "tecnicPrice": 190000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 0,
    "rating": 5.0,
    "reviewCount": 0,
    "isFeatured": true,
    "image": "/products/ĐAI HỖ TRỢ VÙNG CÁNH TAY.png",
    "shortDescription": "Dùng để hỗ trợ cố định và ổn định vùng cổ tay, giúp bảo vệ cổ tay khi vận động, đồng thời hỗ trợ duy trì sự linh hoạt và thoải mái trong quá trình luyện tập hoặc thi đấu.",
    "fullDescription": `### 1. Đối tượng sử dụng phù hợp
**Đai Cổ Tay Wrist Bandage** là sản phẩm hỗ trợ vùng cổ tay và bàn tay với thiết kế đàn hồi, dễ đeo và tháo, giúp tăng sự ổn định cho cổ tay mà vẫn duy trì khả năng vận động linh hoạt.

Sản phẩm phù hợp với:
- **Người thường xuyên vận động, luyện tập thể thao hoặc thi đấu**, cần hỗ trợ và bảo vệ vùng cổ tay.
- **Người muốn giữ cố định cổ tay và bàn tay** trong quá trình vận động.
- **Người cần hỗ trợ hạn chế nguy cơ trật cổ tay** khi thực hiện các hoạt động có tác động lực lên cổ tay.
- **Người ưu tiên sản phẩm co giãn tốt**, dễ cử động và thoải mái khi sử dụng.
- **Người cần một mẫu đai dễ dàng đeo vào và tháo ra**, thuận tiện sử dụng trước và sau khi vận động.

---

### 2. Hướng dẫn sử dụng
**Cách dùng:**
- **Bước 1: Đeo đai vào tay:** Đưa bàn tay vào phần đai và đặt sản phẩm đúng vị trí quanh cổ tay.
- **Bước 2: Điều chỉnh vị trí:** Điều chỉnh đai sao cho phần hỗ trợ ôm sát vùng cổ tay và bàn tay, không bị xoắn hoặc gấp nếp.
- **Bước 3: Điều chỉnh độ ôm:** Kéo và cố định đai với lực vừa phải để tạo sự chắc chắn nhưng vẫn đảm bảo cổ tay và bàn tay có thể cử động thoải mái.
- **Bước 4: Kiểm tra trước khi vận động:** Thực hiện một vài chuyển động nhẹ để đảm bảo đai không bị xê dịch và không gây cản trở các động tác cần thiết.
- **Bước 5: Tháo và bảo quản:** Sau khi sử dụng, tháo đai nhẹ nhàng, vệ sinh theo hướng dẫn của nhà sản xuất và bảo quản nơi khô ráo.

---

### 3. Lưu ý khi sử dụng
- Điều chỉnh đai vừa đủ chặt để tạo sự hỗ trợ, không siết quá mức gây khó chịu hoặc ảnh hưởng tuần hoàn.
- Kiểm tra vị trí đai trước khi vận động hoặc thi đấu để đảm bảo sản phẩm không bị lệch.
- Không sử dụng sản phẩm khi có dấu hiệu hư hỏng hoặc mất độ đàn hồi.
- Nếu xuất hiện cảm giác đau, tê, tím tái, chèn ép hoặc khó chịu bất thường, nên tháo đai và tham khảo ý kiến chuyên gia y tế.
- Sản phẩm có tác dụng hỗ trợ và cố định cổ tay – bàn tay, không thay thế cho chẩn đoán hoặc điều trị y tế chuyên môn.

---

### 4. Mô tả sản phẩm
Đai Cổ Tay Wrist Bandage là sản phẩm hỗ trợ vùng cổ tay và bàn tay, được thiết kế nhằm mang lại sự ổn định chắc chắn nhưng vẫn đảm bảo khả năng vận động linh hoạt.

Sản phẩm chú trọng đến sự tiện dụng và cảm giác thoải mái trong quá trình sử dụng.

Wrist Bandage có thiết kế dễ dàng đeo vào và tháo ra, giúp người dùng nhanh chóng sử dụng trước khi luyện tập, vận động hoặc thi đấu mà không mất nhiều thời gian.

Đai giúp giữ cố định tốt vùng cổ tay và bàn tay, hỗ trợ hạn chế các chuyển động không mong muốn của cổ tay trong quá trình vận động. Nhờ đó, sản phẩm có thể hỗ trợ phòng tránh trật cổ tay trong các hoạt động phù hợp.

Một ưu điểm nổi bật của Wrist Bandage là chất liệu có độ đàn hồi tốt, giúp đai ôm sát cổ tay nhưng vẫn cho phép bàn tay và cổ tay cử động linh hoạt. Điều này đặc biệt thuận tiện với người cần sử dụng đai trong quá trình luyện tập hoặc thi đấu.

Với thiết kế gọn gàng, đàn hồi và dễ sử dụng, Wrist Bandage phù hợp để hỗ trợ cổ tay trong các hoạt động thể thao cũng như vận động hằng ngày.

**Thông số sản phẩm:**
- **Tên sản phẩm:** Wrist Bandage – Đai Cổ Tay
- **Màu sắc:** Đen
- **Xuất xứ:** Thiết kế và sản xuất 100% tại Nhật Bản
- **Chất liệu:** Có độ đàn hồi tốt
- **Vùng hỗ trợ:** Cổ tay và bàn tay
- **Đặc điểm:** Dễ đeo, dễ tháo, co giãn tốt
- **Mục đích sử dụng:** Hỗ trợ cố định và bảo vệ cổ tay khi vận động

---

### 5. Tính năng & Ưu điểm nổi bật
- **Dễ dàng đeo vào và tháo ra:** Thiết kế tiện dụng giúp đeo và tháo nhanh chóng, thuận tiện trước và sau khi luyện tập hoặc thi đấu.
- **Cố định tốt cổ tay và bàn tay:** Đai giúp giữ ổn định vùng cổ tay và bàn tay, hỗ trợ hạn chế những chuyển động không mong muốn trong quá trình vận động.
- **Hỗ trợ phòng tránh trật cổ tay:** Khả năng cố định giúp hỗ trợ bảo vệ cổ tay trước các tác động trong quá trình vận động, đặc biệt khi tham gia các hoạt động thể thao phù hợp.
- **Không ảnh hưởng quá trình vận động:** Chất liệu đàn hồi giúp đai ôm sát nhưng vẫn cho phép cổ tay và bàn tay cử động linh hoạt, không gây cảm giác quá cứng hoặc cồng kềnh.
- **Độ đàn hồi tốt – thoải mái khi sử dụng:** Chất liệu có độ co giãn tốt giúp sản phẩm dễ cử động, ôm vừa vặn và tạo cảm giác thoải mái trong quá trình sử dụng.
- **Đáp ứng tiêu chuẩn chất lượng Nhật Bản:** Sản phẩm đáp ứng yêu cầu về tính tiện dụng và chất lượng cao trong quá trình sử dụng.
- **Màu đen dễ sử dụng:** Màu đen tạo cảm giác gọn gàng, đơn giản và dễ phối hợp khi sử dụng trong luyện tập thể thao hoặc sinh hoạt hằng ngày.

---

### 6. Thông tin liên hệ
Để được tư vấn chi tiết về Đai Cổ Tay Wrist Bandage, cách sử dụng và lựa chọn sản phẩm phù hợp, vui lòng liên hệ:

**CÔNG TY CỔ PHẦN GIẢI PHÁP CÔNG NGHỆ HỖ TRỢ Y TẾ TECNIC (TECNIC Medtech)**
- 🏢 **Địa chỉ:** Tầng 2, Tòa nhà New Skyline, KĐT mới Văn Quán - Yên Phúc, Phường Hà Đông, TP. Hà Nội, Việt Nam.
- 📋 **Mã số thuế:** 0110887948
- 📞 **Điện thoại / Hotline:** 034 84 02466/ 038 988 0369
- 📧 **Email:** tecnic.medtech@gmail.com
- 🌐 **Website:** ytetecnic.vn

*TECNIC Medtech – Kiến tạo để phụng sự*`,
    "specifications": {
      "brand": "Bonbone",
      "origin": "Nhật Bản",
      "warrantyMonths": 12,
      "model": "Wrist Bandage",
      "dimensions": "Freesize ôm sát cổ tay",
      "weight": "45g",
      "material": "Chất liệu có độ đàn hồi tốt, thoáng khí",
      "application": "Dùng để hỗ trợ cố định và ổn định vùng cổ tay, giúp bảo vệ cổ tay khi vận động, đồng thời hỗ trợ duy trì sự linh hoạt và thoải mái trong quá trình luyện tập hoặc thi đấu.",
      "color": "Đen",
      "targetUsers": "Người thường xuyên vận động, luyện tập thể thao; người muốn giữ cố định cổ tay và bàn tay; người cần hạn chế nguy cơ trật cổ tay.",
      "features": [
        "Thiết kế dễ dàng đeo vào và tháo ra, mang lại khả năng hỗ trợ chắc chắn cho vùng cổ tay.",
        "Giúp cố định tốt cổ tay và bàn tay, hỗ trợ ổn định khớp trong quá trình vận động.",
        "Hỗ trợ phòng tránh tình trạng trật cổ tay khi vận động, đồng thời không gây cản trở đáng kể đến các chuyển động cần thiết trong luyện tập và thi đấu.",
        "Sử dụng chất liệu có độ đàn hồi tốt, giúp cổ tay dễ dàng cử động và mang lại cảm giác thoải mái khi sử dụng.",
        "Thiết kế ôm sát, gọn nhẹ, thuận tiện sử dụng trong các hoạt động thể thao và sinh hoạt hằng ngày.",
        "Thiết kế và sản xuất 100% tại Nhật Bản, đảm bảo độ hoàn thiện và chất lượng sản phẩm."
      ]
    }
  },
  {
    "id": 100035,
    "code": "TEC-100035",
    "name": "ĐAI VAI CAO CẤP PAMEDI",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 480000,
    "tecnicPrice": 400000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 168,
    "rating": 4.8,
    "reviewCount": 54,
    "isFeatured": false,
    "image": "/products/ĐAI VAI CAO CẤP PAMEDI.png",
    "shortDescription": "ĐAI VAI CAO CẤP PAMEDI chính hãng Pamedi, xuất xứ Đài Loan.",
    "fullDescription": "Sản phẩm ĐAI VAI CAO CẤP PAMEDI được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Pamedi",
      "origin": "Đài Loan",
      "warrantyMonths": 12,
      "model": "Pamedi",
      "dimensions": "",
      "weight": "",
      "material": "Vải thoáng khí cao cấp",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100036,
    "code": "TEC-100036",
    "name": "ĐAI VAI THƯỜNG",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 396000,
    "tecnicPrice": 330000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 170,
    "rating": 4.8,
    "reviewCount": 55,
    "isFeatured": true,
    "image": "/products/ĐAI VAI THƯỜNG.png",
    "shortDescription": "ĐAI VAI THƯỜNG chính hãng Famedi, xuất xứ Đài Loan.",
    "fullDescription": "Sản phẩm ĐAI VAI THƯỜNG được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Famedi",
      "origin": "Đài Loan",
      "warrantyMonths": 12,
      "model": "Thường",
      "dimensions": "",
      "weight": "",
      "material": "Vải tổng hợp",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100037,
    "code": "TEC-100037",
    "name": "ĐAI HỖ TRỢ DI CHUYỂN",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 276000,
    "tecnicPrice": 230000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 172,
    "rating": 4.8,
    "reviewCount": 56,
    "isFeatured": false,
    "image": "/products/ĐAI HỖ TRỢ DI CHUYỂN.png",
    "shortDescription": "ĐAI HỖ TRỢ DI CHUYỂN chính hãng Famedi, xuất xứ Đài Loan.",
    "fullDescription": "Sản phẩm ĐAI HỖ TRỢ DI CHUYỂN được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Famedi",
      "origin": "Đài Loan",
      "warrantyMonths": 12,
      "model": "Hỗ trợ",
      "dimensions": "",
      "weight": "",
      "material": "Vải dù, đai nẹp",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100038,
    "code": "TEC-100038",
    "name": "ĐAI HỖ TRỢ DI CHUYỂN CÓ DÂY ORBE",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 312000,
    "tecnicPrice": 260000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 174,
    "rating": 4.8,
    "reviewCount": 57,
    "isFeatured": false,
    "image": "/products/ĐAI HỖ TRỢ DI CHUYỂN CÓ DÂY ORBE.png",
    "shortDescription": "ĐAI HỖ TRỢ DI CHUYỂN CÓ DÂY ORBE chính hãng Orbe, xuất xứ Việt Nam.",
    "fullDescription": "Sản phẩm ĐAI HỖ TRỢ DI CHUYỂN CÓ DÂY ORBE được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Orbe",
      "origin": "Việt Nam",
      "warrantyMonths": 12,
      "model": "Có dây",
      "dimensions": "",
      "weight": "",
      "material": "Vải dù, đai nẹp chắc chắn",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100039,
    "code": "TEC-100039",
    "name": "ĐAI HỖ TRỢ DI CHUYỂN KHÔNG DÂY ORBE",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 276000,
    "tecnicPrice": 230000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 176,
    "rating": 4.8,
    "reviewCount": 58,
    "isFeatured": false,
    "image": "/products/ĐAI HỖ TRỢ DI CHUYỂN KHÔNG DÂY ORBE.png",
    "shortDescription": "ĐAI HỖ TRỢ DI CHUYỂN KHÔNG DÂY ORBE chính hãng Orbe, xuất xứ Việt Nam.",
    "fullDescription": "Sản phẩm ĐAI HỖ TRỢ DI CHUYỂN KHÔNG DÂY ORBE được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Orbe",
      "origin": "Việt Nam",
      "warrantyMonths": 12,
      "model": "Không dây",
      "dimensions": "",
      "weight": "",
      "material": "Vải dù, đai nẹp",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100040,
    "code": "TEC-100040",
    "name": "ĐAI LƯNG CAO CẤP (NHIỆT)",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 480000,
    "tecnicPrice": 400000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 178,
    "rating": 4.8,
    "reviewCount": 59,
    "isFeatured": false,
    "image": "/products/ĐAI LƯNG CAO CẤP (NHIỆT).png",
    "shortDescription": "ĐAI LƯNG CAO CẤP (NHIỆT) chính hãng Famedi, xuất xứ Đài Loan.",
    "fullDescription": "Sản phẩm ĐAI LƯNG CAO CẤP (NHIỆT) được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Famedi",
      "origin": "Đài Loan",
      "warrantyMonths": 12,
      "model": "Nhiệt nóng",
      "dimensions": "",
      "weight": "",
      "material": "Vải chun, túi chườm nhiệt",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100041,
    "code": "TEC-100041",
    "name": "ĐAI NẸP CỔ THOÁNG KHÍ",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 720000,
    "tecnicPrice": 600000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 180,
    "rating": 4.8,
    "reviewCount": 60,
    "isFeatured": true,
    "image": "/products/ĐAI NẸP CỔ THOÁNG KHÍ.png",
    "shortDescription": "ĐAI NẸP CỔ THOÁNG KHÍ chính hãng Bonbone, xuất xứ Nhật Bản.",
    "fullDescription": "Sản phẩm ĐAI NẸP CỔ THOÁNG KHÍ được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Bonbone",
      "origin": "Nhật Bản",
      "warrantyMonths": 12,
      "model": "Đai cổ",
      "dimensions": "",
      "weight": "",
      "material": "Vải lưới thoáng khí",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100042,
    "code": "TEC-100042",
    "name": "ĐAI NÂNG VAI",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 1320000,
    "tecnicPrice": 1100000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 182,
    "rating": 4.8,
    "reviewCount": 61,
    "isFeatured": false,
    "image": "/products/ĐAI NÂNG VAI.png",
    "shortDescription": "ĐAI NÂNG VAI chính hãng Bonbone, xuất xứ Nhật Bản.",
    "fullDescription": "Sản phẩm ĐAI NÂNG VAI được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Bonbone",
      "origin": "Nhật Bản",
      "warrantyMonths": 12,
      "model": "Đai vai",
      "dimensions": "",
      "weight": "",
      "material": "Vải chun 4 chiều",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100043,
    "code": "TEC-100043",
    "name": "ĐAI NẸP MẮT CÁ CHÂN",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 1873200,
    "tecnicPrice": 1561000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 184,
    "rating": 4.8,
    "reviewCount": 62,
    "isFeatured": false,
    "image": "/products/ĐAI NẸP MẮT CÁ CHÂN.png",
    "shortDescription": "ĐAI NẸP MẮT CÁ CHÂN chính hãng Bonbone, xuất xứ Nhật Bản.",
    "fullDescription": "Sản phẩm ĐAI NẸP MẮT CÁ CHÂN được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Bonbone",
      "origin": "Nhật Bản",
      "warrantyMonths": 12,
      "model": "Đai chân",
      "dimensions": "",
      "weight": "",
      "material": "Vải định hình cao cấp",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100044,
    "code": "TEC-100044",
    "name": "ĐAI HỖ TRỢ VÙNG CÁNH TAY",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 360000,
    "tecnicPrice": 300000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 186,
    "rating": 4.8,
    "reviewCount": 63,
    "isFeatured": false,
    "image": "/products/ĐAI HỖ TRỢ VÙNG CÁNH TAY.png",
    "shortDescription": "ĐAI HỖ TRỢ VÙNG CÁNH TAY chính hãng Bonbone, xuất xứ Nhật Bản.",
    "fullDescription": "Sản phẩm ĐAI HỖ TRỢ VÙNG CÁNH TAY được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Bonbone",
      "origin": "Nhật Bản",
      "warrantyMonths": 12,
      "model": "Đai cánh tay",
      "dimensions": "",
      "weight": "",
      "material": "Vải tổng hợp",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100045,
    "code": "TEC-100045",
    "name": "ĐAI NẸP KHUỶU TAY",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 660000,
    "tecnicPrice": 550000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 188,
    "rating": 4.8,
    "reviewCount": 64,
    "isFeatured": false,
    "image": "/products/ĐAI NẸP KHUỶU TAY.png",
    "shortDescription": "ĐAI NẸP KHUỶU TAY chính hãng Bonbone, xuất xứ Nhật Bản.",
    "fullDescription": "Sản phẩm ĐAI NẸP KHUỶU TAY được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Bonbone",
      "origin": "Nhật Bản",
      "warrantyMonths": 12,
      "model": "Đai tay",
      "dimensions": "",
      "weight": "",
      "material": "Vải cao cấp Bonbone",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100046,
    "code": "TEC-100046",
    "name": "ĐAI CỐ ĐỊNH ĐẦU GỐI",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 900000,
    "tecnicPrice": 750000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 190,
    "rating": 4.8,
    "reviewCount": 65,
    "isFeatured": true,
    "image": "/products/ĐAI CỐ ĐỊNH ĐẦU GỐI.png",
    "shortDescription": "ĐAI CỐ ĐỊNH ĐẦU GỐI chính hãng Bonbone, xuất xứ Nhật Bản.",
    "fullDescription": "Sản phẩm ĐAI CỐ ĐỊNH ĐẦU GỐI được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Bonbone",
      "origin": "Nhật Bản",
      "warrantyMonths": 12,
      "model": "Đai chân",
      "dimensions": "",
      "weight": "",
      "material": "Vải thoáng khí, nẹp trợ lực",
      "application": "Đai lưng hỗ trợ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100047,
    "code": "TEC-100047",
    "name": "ĐAI HỖ TRỢ CỐ ĐỊNH ĐẦU GỐI",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 780000,
    "tecnicPrice": 650000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 192,
    "rating": 4.8,
    "reviewCount": 66,
    "isFeatured": false,
    "image": "/products/ĐAI HỖ TRỢ CỐ ĐỊNH ĐẦU GỐI.png",
    "shortDescription": "ĐAI HỖ TRỢ CỐ ĐỊNH ĐẦU GỐI chính hãng Bonbone, xuất xứ Nhật Bản.",
    "fullDescription": "Sản phẩm ĐAI HỖ TRỢ CỐ ĐỊNH ĐẦU GỐI được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Bonbone",
      "origin": "Nhật Bản",
      "warrantyMonths": 12,
      "model": "Đai đầu gối",
      "dimensions": "",
      "weight": "",
      "material": "Vải chun cao cấp",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100048,
    "code": "TEC-100048",
    "name": "ĐAI CỐ ĐỊNH KHỚP DƯỚI GỐI",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 720000,
    "tecnicPrice": 600000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 194,
    "rating": 4.8,
    "reviewCount": 67,
    "isFeatured": false,
    "image": "/products/ĐAI CỐ ĐỊNH KHỚP DƯỚI GỐI.png",
    "shortDescription": "ĐAI CỐ ĐỊNH KHỚP DƯỚI GỐI chính hãng Bonbone, xuất xứ Nhật Bản.",
    "fullDescription": "Sản phẩm ĐAI CỐ ĐỊNH KHỚP DƯỚI GỐI được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Bonbone",
      "origin": "Nhật Bản",
      "warrantyMonths": 12,
      "model": "Đai chân",
      "dimensions": "",
      "weight": "",
      "material": "Vải êm ái, chống trượt",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100049,
    "code": "TEC-100049",
    "name": "ĐAI HỖ TRỢ VÙNG CHÂN",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 936000,
    "tecnicPrice": 780000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 196,
    "rating": 4.8,
    "reviewCount": 68,
    "isFeatured": false,
    "image": "/products/ĐAI HỖ TRỢ VÙNG CHÂN.png",
    "shortDescription": "ĐAI HỖ TRỢ VÙNG CHÂN chính hãng Bonbone, xuất xứ Nhật Bản.",
    "fullDescription": "Sản phẩm ĐAI HỖ TRỢ VÙNG CHÂN được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Bonbone",
      "origin": "Nhật Bản",
      "warrantyMonths": 12,
      "model": "Đai chân",
      "dimensions": "",
      "weight": "",
      "material": "Vải Bonbone đặc biệt",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100050,
    "code": "TEC-100050",
    "name": "ĐAI HỖ TRỢ VÙNG BẮP VÀ GÓT CHÂN",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 2520000,
    "tecnicPrice": 2100000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 198,
    "rating": 4.8,
    "reviewCount": 69,
    "isFeatured": false,
    "image": "/products/ĐAI HỖ TRỢ VÙNG BẮP VÀ GÓT CHÂN.png",
    "shortDescription": "ĐAI HỖ TRỢ VÙNG BẮP VÀ GÓT CHÂN chính hãng Bonbone, xuất xứ Nhật Bản.",
    "fullDescription": "Sản phẩm ĐAI HỖ TRỢ VÙNG BẮP VÀ GÓT CHÂN được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Bonbone",
      "origin": "Nhật Bản",
      "warrantyMonths": 12,
      "model": "Bắp và gót chân",
      "dimensions": "",
      "weight": "",
      "material": "Vải cao su tổng hợp",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  },
  {
    "id": 100051,
    "code": "TEC-100051",
    "name": "ĐAI CỐ ĐỊNH CỔ CHÂN",
    "category": "DAI_NEP_KHOP",
    "categoryName": "Đai Nẹp & Khớp",
    "marketPrice": 336000,
    "tecnicPrice": 280000,
    "discountPercent": 17,
    "stock": 50,
    "soldCount": 200,
    "rating": 4.8,
    "reviewCount": 70,
    "isFeatured": true,
    "image": "/products/ĐAI CỐ ĐỊNH CỔ CHÂN.png",
    "shortDescription": "ĐAI CỐ ĐỊNH CỔ CHÂN chính hãng Bonbone, xuất xứ Nhật Bản.",
    "fullDescription": "Sản phẩm ĐAI CỐ ĐỊNH CỔ CHÂN được phân phối chính hãng bởi TECNIC. Đảm bảo chất lượng và uy tín.",
    "specifications": {
      "brand": "Bonbone",
      "origin": "Nhật Bản",
      "warrantyMonths": 12,
      "model": "Đai chân",
      "dimensions": "",
      "weight": "",
      "material": "Vải chun định hình",
      "application": "Thiết bị hỗ trợ chăm sóc, phục hồi"
    }
  }
];
